// Presentation/Settings/SettingsView/Security/AppLockPickerOverlay.swift
//
//  AppLockPickerOverlay.swift
//  TrueScan
//
...
struct AppLockPickerOverlay: View {
    let onFaceID: () -> Void
    let onPasscode: () -> Void
    let onDismiss: () -> Void
    ...
    // MARK: - State
    // Если уже настроен App Lock (есть passcode или включен режим) —
    // показываем ТОЛЬКО 2 пункта: Change passcode + Turn off app lock.
    private var isAppLockConfigured: Bool {
        AppLockPrefs.hasPasscode() || AppLockPrefs.isEnabled()
    }

    // MARK: - Dynamic sheet height
    // Теперь в любом состоянии ровно 2 строки → высота одинаковая.
    private var sheetHeight: CGFloat { 240 }

    private var dismissThreshold: CGFloat { 140.scale }
    private var dismissTravel: CGFloat { 360.scale }
        ...
        ZStack(alignment: .top) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                if isAppLockConfigured {
                    // MARK: - Configured: ONLY Change + Turn Off
                    ...
                    AppLockRow(
                        ...
                        dismissThen(from: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.26) { onChangePasscode() }
                    }
                    ...
                    AppLockRow(
                        ...
                        dismissThen(from: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.26) { onTurnOff() }
                    }
                    .padding(.top, gapRows.scale)
                    .padding(.horizontal, hPadding.scale)
                } else {
                    // MARK: - Not configured: FaceID + Passcode
                    ...
                    AppLockRow(
                        ...
                        dismissThen(from: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.26) { onFaceID() }
                    }
                    .padding(.top, titleToFirstRow.scale)
                    .padding(.horizontal, hPadding.scale)
                    ...
                    AppLockRow(
                        ...
                        dismissThen(from: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.26) { onPasscode() }
                    }
                    .padding(.top, gapRows.scale)
                    .padding(.horizontal, hPadding.scale)
                }
